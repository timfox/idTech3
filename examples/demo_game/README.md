...
3. On load, `autoexec.cfg` runs `demo_features.cfg`, `demo_js.cfg`, `demo_lua.cfg`, `demo_gameplay.cfg`, and `demo_usd.cfg` (FreeUSD console hints + sample `.usda` under `models/` when built with `USE_FREEUSD=ON`). Edit files under `mod/` and rebuild `demo_game_pk3`.
