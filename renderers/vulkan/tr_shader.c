...
#include "tr_shader_remap.inc"
#include "tr_shader_parse_helpers.inc"
...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		else if (!Q_stricmp(token, "specularexponent"))
		{
			...
			/* Must match gen_frag.glsl: max 8192, min 1 */
...
static const infoParm_t infoParms[] = {
	...
	{"vegetation",	0,	SURF_VEGETATION, 0 },	// vegetation wind compute (grass, leaves)
};
