...
void vk_create_render_passes( void )
{
	...
	const qboolean visMrt = ( fboActive && vk.deferredGbufferDirectExport && vk.visibilityBufferDirectExport ) ? qtrue : qfalse;
	uint32_t i;
	...
	colorRefs[4].attachment = VK_ATTACHMENT_UNUSED;
	...
	colorResolveRefs[4].attachment = VK_ATTACHMENT_UNUSED;
	...
	// existing 2D draw pipelines can render here while preserving alpha coverage.
	{
		...
		ui_attachments[2].initialLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		ui_attachments[2].finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	...
	/* Temporal class stamp: same R8 MAX-blend pattern as reactive. */
	if ( vk.temporal_class_image[0] != VK_NULL_HANDLE ) {
		VkAttachmentDescription stampAtt;
		VkAttachmentReference stampRef;
		VkSubpassDescription stampSub;
		VkSubpassDependency stampDeps[2];
		...
		stampDeps[1].dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
		stampDeps[1].srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
		stampDeps[1].dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
		stampDeps[1].dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
		...
		stampDesc.pDependencies = stampDeps;
		stampDesc.dependencyCount = 2;
		VK_CHECK( qvkCreateRenderPass( device, &stampDesc, NULL, &vk.render_pass.temporal_class_stamp ) );
		SET_OBJECT_NAME( vk.render_pass.temporal_class_stamp, "render pass - temporal_class_stamp",
			VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT );
	}
