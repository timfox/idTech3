...
No-op stubs for research neural / paper scaffold renderer paths when built
with USE_EXPERIMENTAL_RENDERERS=OFF. Chocolate paths (MGS/WSP/SqueezeMe,
Hybrid1/Raygun, Arc Blanc) are always linked separately — see
cmake/renderers/VulkanExtensionSources.cmake and docs/NEURAL_RENDERER_PHASES.md.
===========================================================================
*/
...
static void VK_ExpStubLogOnce( const char *tag )
{
	...
	static const char *const s_tags[] = {
		...
		"RenderFormer", "VkSplat", "WPT", "FSA", "GRTX", NULL
	};
	int i;
...
/* --- FSA (Forget Superresolution) --- */
void R_FSA_Init( void ) { VK_ExpStubLogOnce( "FSA" ); (void)VK_ExpStubCvar( "r_fsa", "0", CVAR_ARCHIVE_ND | CVAR_LATCH ); }
...
/* --- GRTX / PathTrace (research RTX; Hybrid1/Raygun are chocolate) --- */
void R_GRTX_Init( void ) { VK_ExpStubLogOnce( "GRTX" ); (void)VK_ExpStubCvar( "r_grtx", "0", CVAR_ARCHIVE_ND | CVAR_LATCH ); }
