...
for MOD in ${MOD_LIST}; do
	echo "[ci_smoke] Starting smoke for fs_game=${MOD}"
	set +e
	"${ENGINE_BIN}" \
		+set fs_game "${MOD}" \
		+set sv_pure 0 \
		...
		+set ttycon 1 \
		+wait 10 \
		+quit >/tmp/ci_smoke_${MOD}.log 2>&1
	STATUS=$?
	set -e

	if [[ ${STATUS} -ne 0 ]]; then
		echo "[ci_smoke] FAILED fs_game=${MOD} (exit ${STATUS}), see /tmp/ci_smoke_${MOD}.log" >&2
		exit ${STATUS}
	fi
	echo "[ci_smoke] PASS fs_game=${MOD}"
done
