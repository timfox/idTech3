...
#ifdef USE_OPUS
	{
		cvar_t *s_enableOpus = Cvar_Get( "s_enableOpus", "1", CVAR_ARCHIVE );
		Cvar_CheckRange( s_enableOpus, "0", "1", CV_INTEGER );
		Cvar_SetDescription( s_enableOpus, "Enable Opus codec support for loading and streaming audio." );
		if ( s_enableOpus->integer ) {
			Com_Printf( "Opus codec enabled.\n" );
			S_CodecRegister( &opus_codec );
		} else {
			Com_Printf( "Opus codec disabled.\n" );
		}
	}
#endif
#ifdef USE_FLAC
	{
		cvar_t *s_enableFlac = Cvar_Get( "s_enableFlac", "1", CVAR_ARCHIVE );
		Cvar_CheckRange( s_enableFlac, "0", "1", CV_INTEGER );
		Cvar_SetDescription( s_enableFlac, "Enable FLAC codec support for loading and streaming audio." );
		if ( s_enableFlac->integer ) {
			Com_Printf( "FLAC codec enabled.\n" );
			S_CodecRegister( &flac_codec );
		} else {
			Com_Printf( "FLAC codec disabled.\n" );
		}
	}
#endif
#ifdef USE_WEBM
	{
		cvar_t *s_enableWebm = Cvar_Get( "s_enableWebm", "1", CVAR_ARCHIVE );
		Cvar_CheckRange( s_enableWebm, "0", "1", CV_INTEGER );
		Cvar_SetDescription( s_enableWebm, "Enable WebM (Opus) codec support for loading and streaming audio." );
		if ( s_enableWebm->integer ) {
			Com_Printf( "WebM codec enabled.\n" );
			S_CodecRegister( &webm_codec );
		} else {
			Com_Printf( "WebM codec disabled.\n" );
		}
	}
#endif
