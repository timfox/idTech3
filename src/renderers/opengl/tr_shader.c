...
static void ParseSkyParms( const char **text ) {
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
			shader.sky.outerbox[i][0] = R_FindImageFile( pathname, imgFlags | IMGFLAG_CLAMPTOEDGE );
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
			shader.sky.innerbox[i][0] = R_FindImageFile( pathname, imgFlags );
		...
		for ( i = 0; i < 6; i++ ) {
			...
			while ( frame < MAX_SKY_ANIMATIONS ) {
				Com_sprintf( pathname, sizeof(pathname), "%s_%s_%d.tga"
					, token, suf[i], frame );
				shader.sky.outerbox[i][frame] = R_FindImageFile( pathname, imgFlags | IMGFLAG_CLAMPTOEDGE );
	...
	if ( strcmp( token, "-" ) ) {
		...
		for ( i = 0; i < 6; i++ ) {
			...
			while ( frame < MAX_SKY_ANIMATIONS ) {
				Com_sprintf( pathname, sizeof(pathname), "%s_%s_%d.tga"
					, token, suf[i], frame );
				shader.sky.innerbox[i][frame] = R_FindImageFile( pathname, imgFlags );
...
/*
====================
processShaderBuffersSequentially

Load and validate shader files one at a time to maintain proper temp memory LIFO order.
...
void R_InitShaders( void ) {
	ri.Printf( PRINT_ALL, "=== OPENGL R_InitShaders CALLED ===\n" );
	...
	if (safeShaderText && shaderTextSize > 0) {
		...
		ri.Printf(PRINT_ALL, "Shader text set up: %d bytes\n", shaderTextSize);
	} else {
		ri.Printf(PRINT_WARNING, "No shader text available from safe loading\n");
	}
