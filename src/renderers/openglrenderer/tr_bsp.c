...
static void R_LoadFogs( const lump_t *l, const lump_t *brushesLump, const lump_t *sidesLump ) {
	...
	for ( i=0 ; i<count ; i++, fogs++) {
		...
		if ( out->originalBrushNumber < 0 || out->originalBrushNumber >= brushesCount ) {
		...
		if ( sidesCount < 6 || firstSide < 0 || firstSide > sidesCount - 6 ) {
		...
		if ( sideNum == -1 ) {
			...
			if ( sideOffset < 0 || sideOffset >= sidesCount ) {
...
void RE_LoadWorldMap( const char *name ) {
	...
	if ( size < (int32_t)sizeof( dheader_t ) ) {
