...
#else
/* NanoSVG fallback when librsvg is unavailable - enables SVG graphics on all platforms */
...
void R_LoadSVG( const char *filename, byte **pic, int *width, int *height ) {
	...
	*pic = NULL;
	...
	if ( maxFileBytes < 64 * 1024 ) maxFileBytes = 64 * 1024;
	if ( maxFileBytes > 32 * 1024 * 1024 ) maxFileBytes = 32 * 1024 * 1024;
	maxRasterSize = Cvar_VariableIntegerValue( "r_svgMaxRasterSize" );
	if ( maxRasterSize < 64 ) maxRasterSize = 64;
	if ( maxRasterSize > 8192 ) maxRasterSize = 8192;
	rasterScale = Cvar_VariableValue( "r_svgRasterScale" );
	if ( rasterScale < 0.1f ) rasterScale = 0.1f;
	if ( rasterScale > 8.0f ) rasterScale = 8.0f;
	...
	/* nsvgParse modifies the buffer; ensure null termination */
	{
	...
	/* NanoSVG outputs RGBA; engine expects RGBA. No conversion needed. */

	nsvgDeleteRasterizer( rast );
	nsvgDelete( image );

	*pic = out;
	if ( width ) *width = targetW;
	if ( height ) *height = targetH;
}
