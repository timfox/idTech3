...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	if ( vk.pbrActive && ( physicalAlbedo || stage->physicalMapType != PHYS_NONE ) ) {
		...
		if ( !stage->physicalMap && physicalAlbedo ) {
			for ( i = 0; i < (int)ARRAY_LEN( textureMapTypes ); i++ ) {
...
static void ParseSkyParms( const char **text ) {
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
			shader.sky.outerbox[i][0] = R_FindImageFile( pathname, imgFlags | IMGFLAG_CLAMPTOEDGE, 0 );
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
			shader.sky.innerbox[i][0] = R_FindImageFile( pathname, imgFlags, 0 );
		...
		for ( i = 0; i < 6; i++ ) {
			...
			while ( frame < MAX_SKY_ANIMATIONS ) {
				Com_sprintf( pathname, sizeof(pathname), "%s_%s_%d.tga"
					, token, suf[i], frame );
				shader.sky.outerbox[i][frame] = R_FindImageFile( pathname, imgFlags | IMGFLAG_CLAMPTOEDGE, 0 );
	...
	if ( strcmp( token, "-" ) ) {
		...
		for ( i = 0; i < 6; i++ ) {
			...
			while ( frame < MAX_SKY_ANIMATIONS ) {
				Com_sprintf( pathname, sizeof(pathname), "%s_%s_%d.tga"
					, token, suf[i], frame );
				shader.sky.innerbox[i][frame] = R_FindImageFile( pathname, imgFlags, 0 );
