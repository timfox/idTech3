...
static const char *R_LoadImage( const char *name, byte **pic, int *width, int *height )
{
	...
	*height = 0;
	...
	if ( *ext )
	{
		...
		// If shader explicitly asked for foo.exr and it's missing: try same basename other extensions.
		...
		// Otherwise fall back to trying without the extension.
	...
	// Fallback: try all supported formats (including svg/pcx/bmp)
	for ( i = 0; i < numImageLoaders; i++ )
	{
		if ( i == orgLoader )
			continue;
...
static image_t *R_FindCriticalFallbackImage( const char *name )
{
	char base[ MAX_QPATH ];
	const char *leaf;
	...
	// white/black are always valid fallbacks in Vulkan (created at init)
	...
	if ( !Q_stricmp( base, "*black" ) || !Q_stricmp( base, "black" ) ) {
		return tr.blackImage ? tr.blackImage : NULL;
	}

	// console: reuse checker or white
...
image_t	*R_FindImageFile( const char *name, imgFlags_t flags, uint32_t type )
{
	...
	if ( pic == NULL ) {
		// For critical built-in shaders/images, reuse procedural fallbacks.
		image = R_FindCriticalFallbackImage( name );
		if ( image ) {
			return image;
		}
		return NULL;
...
static void R_CreateBuiltinImages( void ) {
	...
	ri.Printf(PRINT_ALL, "R_CreateBuiltinImages: Creating black image\n");
	{
		...
		tr.blackImage = R_CreateImage( "*black", NULL, (byte *)rgba, 1, 1, IMGFLAG_NONE, 0, 0 );
	}
	...
	ri.Printf(PRINT_ALL, "R_CreateBuiltinImages: Creating white image\n");
	{
		...
		tr.whiteImage = R_CreateImage( "*white", NULL, (byte *)rgba, 1, 1, IMGFLAG_NONE, 0, 0 );
	}
