...
qboolean vk_create_main_render_pass(void) {
    ...
    attachments[0].format = vk.renderFormat;
...
void vk_begin_bloom_extract_render_pass(void) {
    if (!vk_validate_handle(vk.bloomExtractRenderPass, "bloom extract render pass") ||
        !vk_validate_handle(vk.bloomExtractFramebuffer, "bloom extract framebuffer")) {
        return;
    }
    ...
    VkRenderPassBeginInfo render_pass_info = {
        .sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
        .renderPass = vk.bloomExtractRenderPass,
        .framebuffer = vk.bloomExtractFramebuffer,
        .renderArea = {
            .offset = {0, 0},
...
void vk_begin_blur_render_pass(uint32_t index) {
    if (!vk_bounds_check(index, ARRAY_LEN(vk.blurRenderPasses), "blur render pass") ||
        !vk_bounds_check(index, ARRAY_LEN(vk.blurFramebuffers), "blur framebuffer")) {
        return;
    }
    ...
    VkRenderPassBeginInfo render_pass_info = {
        .sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
        .renderPass = vk.blurRenderPasses[index],
        .framebuffer = vk.blurFramebuffers[index],
        .renderArea = {
            .offset = {0, 0},
            .extent = {vk.renderWidth, vk.renderHeight}
        },
