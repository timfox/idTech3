...
static qboolean R_LoadMD3( model_t *mod, int lod, void *buffer, int fileSize, const char *mod_name ) {
	...
	size_t sizeSz = (size_t)size;

	if ( (size_t)hdr->ofsFrames > sizeSz || (size_t)hdr->ofsTags > sizeSz || (size_t)hdr->ofsSurfaces > sizeSz ) {
	...
	if ( (size_t)hdr->ofsFrames + (size_t)hdr->numFrames * sizeof( md3Frame_t ) > fileSizeSz ) {
		ri.Printf( PRINT_WARNING, "%s: %s has corrupted header\n", __func__, mod_name );
		return qfalse;
	}
	if ( (size_t)hdr->ofsTags + (size_t)hdr->numTags * (size_t)hdr->numFrames * sizeof( md3Tag_t ) > fileSizeSz ) {
		ri.Printf( PRINT_WARNING, "%s: %s has corrupted header\n", __func__, mod_name );
		return qfalse;
	}
	if ( (size_t)hdr->ofsSurfaces + ( hdr->numSurfaces ? 1 : 0 ) * sizeof( md3Surface_t ) > fileSizeSz ) {
		ri.Printf( PRINT_WARNING, "%s: %s has corrupted header\n", __func__, mod_name );
		return qfalse;
	}
	...
	for ( i = 0 ; i < hdr->numSurfaces; i++) {
		...
		if ( (size_t)surf->ofsEnd > fileSizeSz || (((byte*)surf - (byte*)hdr) + (size_t)surf->ofsEnd) > fileSizeSz ) {
			ri.Printf( PRINT_WARNING, "%s: %s has corrupted surface header\n", __func__, mod_name );
			return qfalse;
		}
		if ( (size_t)surf->ofsTriangles > fileSizeSz || (size_t)surf->ofsShaders > fileSizeSz || (size_t)surf->ofsSt > fileSizeSz || (size_t)surf->ofsXyzNormals > fileSizeSz ) {
			ri.Printf( PRINT_WARNING, "%s: %s has corrupted surface header\n", __func__, mod_name );
			return qfalse;
		}
		if ( (size_t)surf->ofsTriangles + (size_t)surf->numTriangles * sizeof( md3Triangle_t ) > fileSizeSz ) {
			ri.Printf( PRINT_WARNING, "%s: %s has corrupted surface header\n", __func__, mod_name );
			return qfalse;
		}
		if ( (size_t)surf->ofsShaders + (size_t)surf->numShaders * sizeof( md3Shader_t ) > fileSizeSz || surf->numShaders > (1<<20) ) {
			ri.Printf( PRINT_WARNING, "%s: %s has corrupted surface header\n", __func__, mod_name );
			return qfalse;
		}
		if ( (size_t)surf->ofsSt + (size_t)surf->numVerts * sizeof( md3St_t ) > fileSizeSz ) {
			ri.Printf( PRINT_WARNING, "%s: %s has corrupted surface header\n", __func__, mod_name );
			return qfalse;
		}
		if ( (size_t)surf->ofsXyzNormals + (size_t)surf->numVerts * sizeof( md3XyzNormal_t ) > fileSizeSz ) {
			ri.Printf( PRINT_WARNING, "%s: %s has corrupted surface header\n", __func__, mod_name );
			return qfalse;
		}
...
static qboolean R_LoadMDR( model_t *mod, void *buffer, int filesize, const char *mod_name ) 
{
	...
	if(pinmodel->numBones < 0 ||
		(size_t)sizeof(*mdr) + (size_t)pinmodel->numFrames * (sizeof(*frame) + (size_t)(pinmodel->numBones - 1) * sizeof(*frame->bones)) > sizeSz)
	{
