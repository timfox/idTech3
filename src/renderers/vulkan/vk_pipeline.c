...
// Basic SPIR-V shader loading for runtime shader module creation
static VkShaderModule vk_load_spirv_shader(const char *filename) {
	char filepath[1024];
	void *spirv_data;
	int file_len;
	...
	if (file_len % 4 != 0) {
		ri.Printf(PRINT_WARNING, "SPIR-V file size not aligned: %s (%d bytes)\n", filepath, file_len);
	...
	VkShaderModuleCreateInfo createInfo = {
		...
		.codeSize = file_len,
		.pCode = (uint32_t *)spirv_data
	};

	VkShaderModule shaderModule;
	VkResult result = qvkCreateShaderModule(vk.device, &createInfo, NULL, &shaderModule);
	if (result != VK_SUCCESS) {
		ri.Printf(PRINT_ERROR, "Failed to create shader module from %s: %s\n", filepath, vk_result_string(result));
		ri.FS_FreeFile(spirv_data);
		return VK_NULL_HANDLE;
	}

	ri.FS_FreeFile(spirv_data);
	ri.Printf(PRINT_DEVELOPER, "Loaded SPIR-V shader: %s\n", filename);
	return shaderModule;
}
...
void vk_create_shader_modules(void)
{
	...
	vk.modules.color_fs = vk_load_spirv_shader("color_frag");

	// Dot shader (for backwards compatibility)
	vk.modules.dot_vs = vk.modules.vert.gen[0][0][0][0][0];
	vk.modules.dot_fs = vk_load_spirv_shader("dot_frag");
...
if (getenv("VK_VERBOSE_PIPELINE_LOGS")) {
    ...
    // Emit a metrics increment for pipeline allocation
    vk_metrics_increment_pipeline_alloc();
