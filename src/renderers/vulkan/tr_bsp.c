...
static const int lightmapFlags = IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_LIGHTMAP | IMGFLAG_NOSCALE | IMGFLAG_MIPMAP;
...
static qboolean R_ParseSpawnVars( char *spawnVarChars, int maxSpawnVarChars, int *numSpawnVars, char *spawnVars[MAX_SPAWN_VARS][2] )
{
	...
	*numSpawnVars = 0;
	...
	while ( 1 ) {  
		...
		Q_strncpyz( spawnVarChars + numSpawnVarChars, keyname, keyLength );
		spawnVars[ *numSpawnVars ][0] = spawnVarChars + numSpawnVarChars;
		numSpawnVarChars += keyLength;
		Q_strncpyz( spawnVarChars + numSpawnVarChars, com_token, tokenLength );
...
vk_material_parser_load_map_materials(s_worldData.baseName);
	...
	vk_generate_light_directions();
			...
			if ( vk.cubemapActive )
			{
				...
				if ( !tr.numCubemaps ) {
		for ( int j = 0; j < (int)ARRAY_LEN(cubemapEntities); j++ )
					{
