...
{
	char **shaderFiles = NULL, **mtrFiles = NULL;
	...
	for (int dir_idx = 0; dir_idx < NUM_SHADER_DIRS; dir_idx++) {
		...
		if (!mtrFiles) {
			mtrFiles = ri.FS_ListFiles(dir, ".mtrx", &numMtrFiles);
		}
		if (shaderFiles && mtrFiles) break; // Found files in preferred directory
	}

	if ((!shaderFiles || !numShaderFiles) && (!mtrFiles || !numMtrFiles)) {
		ri.Printf(PRINT_WARNING, "WARNING: no shader files found in /shaders or /scripts directories\n");
		return;
	}
	...
	sum = 0;
	sum += loadShaderBuffers( mtrFiles, numMtrFiles, xbuffers, shaderDirectory );
