/*
===========================================================================
Light Clustering Stub (GL2 renderer)
...
void R_BuildLightClusters( void ) {
	...
	for (int li = 0; li < tr.refdef.num_dlights && li < LC_MAX_LIGHTS; ++li) {
		...
		// Transform to view space (manual)
		vec3_t vpos;
		for (int a = 0; a < 3; ++a) {
	...
	qglBindBufferBase( GL_SHADER_STORAGE_BUFFER, 6, lcHeaderBuffer );
	...
	qglBindBufferBase( GL_SHADER_STORAGE_BUFFER, 7, lcIndexBuffer );
