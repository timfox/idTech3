...
static qboolean IQM_CheckRange( iqmHeader_t *header, int offset,
	...
	return ( count <= 0 ||
		 offset <= 0 ||
		 (uint32_t)offset > header->filesize ||
		 (uint32_t)(offset + count * size) < (uint32_t)offset || /* overflow */
		 (uint32_t)(offset + count * size) > header->filesize );
}
...
qboolean R_LoadIQM( model_t *mod, void *buffer, int filesize, const char *mod_name ) {
	...
	LL( header->filesize );
	if( header->filesize > (uint32_t)filesize || header->filesize > (16u<<20) ) {
		return qfalse;
	}
	...
	if ( header->num_joints )
	{
		...
		for( i = 0; i < (int)header->num_joints; i++, joint++ ) {
			...
			if( joint->parent < -1 ||
				...
				(uint32_t)joint->name >= header->num_text ) {
				return qfalse;
			}
