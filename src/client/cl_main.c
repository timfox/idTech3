...
static void CL_Record_f( void ) {
	...
	if ( Cmd_Argc() == 2 ) {
		...
		if ( *ext ) {
			...
			Q_snprintf( demoExt, sizeof(demoExt), "%s%d", DEMOEXT, OLD_PROTOCOL_VERSION );
			if ( Q_stricmp( ext, demoExt ) == 0 ) {
				*(strrchr( demoName, '.' )) = '\0';
			} else {
				// check both protocols
				Q_snprintf( demoExt, sizeof(demoExt), "%s%d", DEMOEXT, NEW_PROTOCOL_VERSION );
				if ( Q_stricmp( ext, demoExt ) == 0 ) {
					*(strrchr( demoName, '.' )) = '\0';
				}
			}
...
static void CL_SendPureChecksums( void ) {
	...
	len = Q_snprintf( cMsg, sizeof(cMsg), "cp %d ", cl.serverId );
...
void CL_PacketEvent( const netadr_t *from, msg_t *msg ) {
	...
	if ( !NET_CompareAdr( from, &clc.netchan.remoteAddress ) ) {
		if ( com_developer->integer ) {
			Com_Printf(
				"%s: sequenced packet without connection (expected from %s, state=%d)\n",
				NET_AdrToStringwPort( from ),
				NET_AdrToStringwPort( &clc.netchan.remoteAddress ),
				cls.state );
		}
		// Ignore sequenced packets that are not from the connected server
		return;
	}
...
static void CL_GlobalServers_f( void ) {
	...
	if ( masterNum == 0 ) {
		...
		for ( i = 1; i <= MAX_MASTER_SERVERS; i++ ) {
			Q_snprintf( command, sizeof(command), "sv_master%d", i );
	...
	Q_snprintf( command, sizeof(command), "sv_master%d", masterNum );
	masteraddress = Cvar_VariableString( command );
