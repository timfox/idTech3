...
Modern implementation using Rodrigues' rotation formula.
More efficient and numerically stable than constructing full matrices.
===============
*/
void RotatePointAroundVector( vec3_t dst, const vec3_t dir, const vec3_t point,
	...
	// (k x v)
	CrossProduct( k, point, temp );

	// k . v
	dot = DotProduct( k, point );

	// v*cos(theta) + (k x v)*sin(theta) + k*(k . v)*(1 - cos(theta))
	dst[0] = point[0] * c + temp[0] * s + k[0] * dot * ( 1.0f - c );
	dst[1] = point[1] * c + temp[1] * s + k[1] * dot * ( 1.0f - c );
	dst[2] = point[2] * c + temp[2] * s + k[2] * dot * ( 1.0f - c );
}
