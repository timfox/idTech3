...
# full CTest on Vulkan, optional map-load sanity when GAME_BASE is set.
#
# Usage:
...
if [ "$SKIP_VALIDATE" -eq 0 ]; then
  echo "--- 1/3 validate_ci_build (shaders, Vulkan, smoke, renderer regression) ---"
  ./scripts/validate_ci_build.sh
  echo ""
else
  echo "--- 1/3 validate_ci_build SKIPPED ---"
  echo ""
fi

echo "--- 2/3 CTest (Vulkan build dir) ---"
...
if [ -n "${GAME_BASE:-}" ]; then
  echo "--- 3/3 Map load sanity (GAME_BASE) ---"
  GAME_BASE="$GAME_BASE" ./scripts/renderer_regression_maps.sh
  echo ""
else
  echo "--- 3/3 Map load sanity SKIPPED (set GAME_BASE to enable) ---"
  echo ""
fi
