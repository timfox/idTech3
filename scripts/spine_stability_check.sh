...
grep -q 'seta r_renderMode 3' config/modern_vulkan_stable.cfg || fail "stable must be Unified Clustered mode 3"
grep -q 'seta r_taa 0' config/modern_vulkan_stable.cfg || fail "stable must disable TAA"
grep -q 'seta r_oit 1' config/modern_vulkan_stable.cfg || fail "stable must enable production WBOIT"
grep -q 'seta r_oitForwardPlus 1' config/modern_vulkan_stable.cfg || fail "stable must light OIT through clustered Forward+"
...
grep -q 'clamp( samp.opacity, 0.0, 0.999 )' renderers/vulkan/shaders/glsl/oit_accum.frag || fail "WBOIT alpha clamp missing"
