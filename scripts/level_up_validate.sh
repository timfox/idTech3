...
echo "--- 1/7 MSVC qcommon source parity ---"
./tests/scripts/test_msvc_qcommon_parity.sh
echo ""
...
echo "--- 4/7 OpenArena launcher regression ---"
...
if [ -f "$RELEASE_DIR/idtech3" ] || [ -f "$RELEASE_DIR/idtech3.exe" ] || \
   [ -f "$RELEASE_DIR/idtech3.x86_64" ] || [ -f "$RELEASE_DIR/idtech3.x86_64.exe" ]; then
	...
	echo "--- 5/7 smoke test SKIPPED (no release client; run compile_engine.sh vulkan) ---"
	echo ""
fi

SPEC_REPO="${SPEC_ENERGY_REPO:-$PROJECT_ROOT/external/flux_spec_energy}"
if [ -x "$SCRIPT_DIR/spec_energy_check.sh" ] && [ -d "$SPEC_REPO" ]; then
	echo "--- 6/7 spec_energy static check ---"
	./scripts/spec_energy_check.sh "$SPEC_REPO"
	echo ""
else
	echo "--- 6/7 spec_energy SKIPPED (set SPEC_ENERGY_REPO or clone under external/flux_spec_energy) ---"
	echo ""
fi
