...
def compile_shader(stage, source, array_name, binding_expr=None, defines="", rtx_collect=False, grtx_collect=False, pathtrace_collect=False, hybrid1_collect=False, raygun_collect=False, surfel_collect=False, rcgi_collect=False, av_collect=False, shs_collect=False, rgi_collect=False, rfx_collect=False):
    global task_counter
    ...
    if rgi_collect:
        rgi_spv_bytes[array_name] = tmp_spv.read_bytes()
    if rfx_collect:
        rfx_spv_bytes[array_name] = tmp_spv.read_bytes()
...
compile_shader("comp", "temporal/depth_history_copy.comp", "temporal_depth_history_copy_cs", binding_expr="vk.modules.temporal_depth_history_copy_cs")
compile_shader("comp", "temporal/depth_reject_stats.comp", "temporal_depth_reject_stats_cs", binding_expr="vk.modules.temporal_depth_reject_stats_cs")
...
compile_shader("comp", "ambient_visibility/av_gtao.comp", "av_gtao_cs", av_collect=True)
compile_shader("comp", "ambient_visibility/av_rtao.comp", "av_rtao_cs", av_collect=True)
...
compile_shader("comp", "raster_gi/rgi_probe_sample.comp", "rgi_probe_sample_cs", rgi_collect=True)
compile_shader("comp", "raster_gi/rgi_ssgi.comp", "rgi_ssgi_cs", rgi_collect=True)
compile_shader("comp", "raster_gi/rgi_resolve.comp", "rgi_resolve_cs", rgi_collect=True)
compile_shader("comp", "raster_gi/rgi_clipmap_sample.comp", "rgi_clipmap_sample_cs", rgi_collect=True)

compile_shader("comp", "raster_fx/gp_update.comp", "gp_update_cs", rfx_collect=True)
compile_shader("comp", "raster_fx/gp_soft_splat.comp", "gp_soft_splat_cs", rfx_collect=True)
compile_shader("comp", "raster_fx/dd_apply.comp", "dd_apply_cs", rfx_collect=True)
compile_shader("comp", "raster_fx/distortion_apply.comp", "distortion_apply_cs", rfx_collect=True)
...
def write_vk_surfel_gi_spirv_inc():
    """Emit renderers/vulkan/extensions/rtx/vk_surfel_gi_spirv.inc for Surfel GI (GIBS)."""
    root = Path(os.environ.get("PROJECT_ROOT", "")).resolve()
    if not root or not root.is_dir():
        sys.exit("PROJECT_ROOT must be set for Surfel GI SPIR-V embed")
    out_path = root / "renderers/vulkan/extensions/rtx/vk_surfel_gi_spirv.inc"
    mapping = [
    ...
    lines.append("")
    for src_name, c_array, size_macro in mapping:
        if src_name not in surfel_spv_bytes:
            sys.exit(f"Missing Surfel GI SPIR-V blob for {src_name}")
        data = surfel_spv_bytes[src_name]
        lines.append(f"/* {src_name}.spv {len(data)} bytes */")
        lines.append(f"static const uint8_t {c_array}[] = {{")
        for offset in range(0, len(data), 16):
            chunk = data[offset:offset + 16]
...
def write_vk_ambient_visibility_spirv_inc():
    """Emit embedded SPIR-V for Ambient Visibility."""
    root = Path(os.environ.get("PROJECT_ROOT", "")).resolve()
    if not root or not root.is_dir():
        sys.exit("PROJECT_ROOT must be set for Ambient Visibility SPIR-V embed")
    out_path = root / "renderers/vulkan/vk_ambient_visibility_spirv.inc"
    mapping = [
        ("av_gtao_cs", "vk_av_gtao_cs_spv", "VK_AV_GTAO_CS_SPV_SIZE"),
        ("av_rtao_cs", "vk_av_rtao_cs_spv", "VK_AV_RTAO_CS_SPV_SIZE"),
        ("av_temporal_cs", "vk_av_temporal_cs_spv", "VK_AV_TEMPORAL_CS_SPV_SIZE"),
        ("av_filter_cs", "vk_av_filter_cs_spv", "VK_AV_FILTER_CS_SPV_SIZE"),
        ("av_composite_cs", "vk_av_composite_cs_spv", "VK_AV_COMPOSITE_CS_SPV_SIZE"),
    ]
    lines = ["/* Auto-generated by scripts/compile_shaders.sh — do not edit by hand */", ""]
    for src_name, c_array, size_macro in mapping:
        if src_name not in av_spv_bytes:
            sys.exit(f"Missing Ambient Visibility SPIR-V blob for {src_name}")
        data = av_spv_bytes[src_name]
        lines.append(f"/* {src_name}.spv {len(data)} bytes */")
        lines.append(f"static const uint8_t {c_array}[] = {{")
        for offset in range(0, len(data), 16):
            chunk = data[offset:offset + 16]
...
def write_vk_raster_gi_spirv_inc():
    """Emit embedded SPIR-V for Raster Ultra probe GI + SSGI."""
    root = Path(os.environ.get("PROJECT_ROOT", "")).resolve()
    if not root or not root.is_dir():
        sys.exit("PROJECT_ROOT must be set for Raster GI SPIR-V embed")
    out_path = root / "renderers/vulkan/vk_raster_gi_spirv.inc"
    mapping = [
        ("rgi_probe_sample_cs", "vk_rgi_probe_sample_cs_spv", "VK_RGI_PROBE_SAMPLE_CS_SPV_SIZE"),
        ("rgi_ssgi_cs", "vk_rgi_ssgi_cs_spv", "VK_RGI_SSGI_CS_SPV_SIZE"),
        ("rgi_resolve_cs", "vk_rgi_resolve_cs_spv", "VK_RGI_RESOLVE_CS_SPV_SIZE"),
        ("rgi_clipmap_sample_cs", "vk_rgi_clipmap_sample_cs_spv", "VK_RGI_CLIPMAP_SAMPLE_CS_SPV_SIZE"),
    ]
    lines = ["/* Auto-generated by scripts/compile_shaders.sh — do not edit by hand */", ""]
    for src_name, c_array, size_macro in mapping:
        if src_name not in rgi_spv_bytes:
            sys.exit(f"Missing Raster GI SPIR-V blob for {src_name}")
        data = rgi_spv_bytes[src_name]
        lines.append(f"/* {src_name}.spv {len(data)} bytes */")
        lines.append(f"static const uint8_t {c_array}[] = {{")
        for offset in range(0, len(data), 16):
            chunk = data[offset:offset + 16]
...
write_vk_raster_gi_spirv_inc()

def write_vk_raster_fx_spirv_inc():
    """Emit embedded SPIR-V for Raster Ultra 1.4 FX (particles, decals, distortion)."""
    root = Path(os.environ.get("PROJECT_ROOT", "")).resolve()
    if not root or not root.is_dir():
        sys.exit("PROJECT_ROOT must be set for Raster FX SPIR-V embed")
    out_path = root / "renderers/vulkan/vk_raster_fx_spirv.inc"
    mapping = [
        ("gp_update_cs", "vk_gp_update_cs_spv", "VK_GP_UPDATE_CS_SPV_SIZE"),
        ("gp_soft_splat_cs", "vk_gp_soft_splat_cs_spv", "VK_GP_SOFT_SPLAT_CS_SPV_SIZE"),
        ("dd_apply_cs", "vk_dd_apply_cs_spv", "VK_DD_APPLY_CS_SPV_SIZE"),
        ("distortion_apply_cs", "vk_distortion_apply_cs_spv", "VK_DISTORTION_APPLY_CS_SPV_SIZE"),
    ]
    lines = ["/* Auto-generated by scripts/compile_shaders.sh — do not edit by hand */", ""]
    for src_name, c_array, size_macro in mapping:
        if src_name not in rfx_spv_bytes:
            sys.exit(f"Missing Raster FX SPIR-V blob for {src_name}")
        data = rfx_spv_bytes[src_name]
        lines.append(f"/* {src_name}.spv {len(data)} bytes */")
        lines.append(f"static const uint8_t {c_array}[] = {{")
        for offset in range(0, len(data), 16):
            chunk = data[offset:offset + 16]
