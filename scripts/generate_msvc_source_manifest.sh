...
echo "${MANIFEST}"
python3 -c "import json,sys; d=json.load(open(sys.argv[1])); print('groups:', ', '.join(f'{k}={len(v)}' for k,v in sorted(d.items()))); print('total:', sum(len(v) for v in d.values()))" "${MANIFEST}"
