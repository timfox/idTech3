...
grep -q 'VK_RasterUltra_PrintStatus' "$ROOT/renderers/vulkan/diagnostics/tr_init_diagnostics.inc" || { echo "FAIL status line missing"; fail=1; }
...
# Certified stable must remain mode 3 and not set r_rasterUltra 1
grep -q 'r_renderMode 3' "$ROOT/config/modern_vulkan_stable.cfg" || { echo "FAIL stable mode not 3"; fail=1; }
