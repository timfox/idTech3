...
<p><h2>Build Instructions</h2>

<h3>Modern CMake workflow (recommended)</h3>
...
<ul><li>`-DENABLE_ASAN=ON -DENABLE_UBSAN=ON` (debug): Address/UB sanitizers</li>
<li>`-DENABLE_VALGRIND=ON` (debug): add debug symbols/flags for Valgrind runs</li>
<li>`-DENABLE_DRMEMORY=ON` (Windows): enable Dr. Memory support</li>
<li>`-DENABLE_COVERAGE=ON` (gcc/clang, debug): add `--coverage` flags and expose `coverage` target</li>

</ul>Coverage workflow (requires `gcovr` in `PATH`):

```
cmake -S . -B build-coverage -DCMAKE_BUILD_TYPE=Debug -DENABLE_COVERAGE=ON
cmake --build build-coverage
cmake --build build-coverage --target coverage
<h1>outputs coverage.html and coverage.xml in the build dir</h1>
```
