...
grep -q 'vk_oit_lab_finalize_frame\|vk_cert_readback_record_oit_snapshot' "$LAB" || fail "2.6C deferred snapshot path missing"
grep -q 'vk_oit_certify_note_gpu_core_complete' "$LAB" || fail "LIVE_FULL note missing"
grep -q 'r_oitCertSoakMinutes\|oit_soak_wboit' "$LAB" || fail "soak chain missing"
...
grep -q 'vk_cert_readback_record_oit_snapshot' "$RB_C" || fail "snapshot record missing in readback"
grep -q 'vk_oit_lab_finalize_frame' "$ROOT/renderers/vulkan/vk_frame_submit.c" || fail "finalize not hooked in frame submit"
grep -q 'oit_accum_image' "$ROOT/renderers/vulkan/vk_attachments.c" || fail "oit accum attachment"
grep -A2 'oit_accum_image' "$ROOT/renderers/vulkan/vk_attachments.c" | grep -q 'TRANSFER_SRC' || fail "OIT accum needs TRANSFER_SRC for snapshots"
pass "lab + wiring"
...
grep -q '2.6C\|deferred\|snapshot\|PRODUCTION' "$DOC" || fail "doc must mention 2.6C"
...
echo "All Phase 2.6A/2.6B/2.6C evidence / fixture / live-exec gates passed."
