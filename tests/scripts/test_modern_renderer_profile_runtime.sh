...
source_checks() {
	...
	check_grep "$stable" 'seta r_surfelGi 1' "stable profile requests default Surfel GI"
	check_grep "$stable" 'seta r_surfelGiDensity 2' "stable profile uses balanced Surfel GI density"
	check_grep "$stable" 'seta r_surfelGi_hybrid1Fusion 1' "stable profile keeps Surfel/Hybrid1 fusion enabled"
	check_grep "$stable" 'seta r_visibilityBuffer 1' "stable profile enables production visibility buffer"
	check_grep "$stable" 'seta r_visibilityBufferFill 2' "stable profile prefers PrimID MRT visibility fill"
	check_grep "$stable" 'seta r_materialClassify 1' "stable profile enables material classify"
	check_grep "$stable" 'seta r_deferredMaterialClassify 1' "stable profile consumes material classes in deferred lighting"
	check_grep "$stable" 'seta r_virtualGeometry 1' "stable profile enables virtual geometry"
	check_grep "$stable" 'seta r_meshlets 1' "stable profile enables meshlets"
	check_grep "$stable" 'seta r_meshletsMdiDraw 1' "stable profile enables meshlet MDI draws"
	check_grep "$stable" 'seta r_meshletsLod 1' "stable profile enables meshlet screen LOD"
	...
	check_grep "$rtx" 'seta r_surfelGi 1' "RTX overlay enables Surfel GI"
	check_grep "$hybrid" 'exec modern_vulkan.cfg' "Hybrid1 overlay inherits modern profile"
	check_grep "$hybrid" 'seta r_surfelGi 1' "Hybrid1 overlay enables Surfel GI fusion"
	...
	check_grep "$tr_diag" 'graph     : observed=%u compiled=%u deps=%u frameViol=%u totalViol=%u' "renderer_status reports render graph health"
	...
	check_grep "$tr_diag" 'rendergraph: observed=%u compiled=%u deps=%u frameViol=%u totalViol=%u' "renderer_subsystems reports render graph counts"
