...
grep -q 'SV_ApplyClassicBaseq3ServerCvars' runtime/server/core/sv_init.c || fail 'server classic cvars missing'
grep -q 'cl_engineSprites 0' config/classic_baseq3.cfg || fail 'classic cfg must disable cl_engineSprites'
grep -q 'retail entity number mapping active' runtime/server/gameplay/sv_game.c || fail 'qvm entity num mapping missing'
grep -q 'SV_UseLegacyNativeEntityNums' runtime/server/gameplay/sv_game.c || fail 'entity num mapping helper missing'
grep -q '!gvm->dllHandle' runtime/server/gameplay/sv_game.c || fail 'qvm entity mapping must use dllHandle'
grep -q 'SV_EngineEntityNumToGame' runtime/server/gameplay/sv_bot.c || fail 'botlib trace entity mapping missing'
grep -q 'legacy_bsp_trace_t' runtime/server/gameplay/sv_bot.c || fail 'retail botlib bsp_trace layout missing'
grep -q 'legacy_trace_t' runtime/server/gameplay/sv_game.c || fail 'server retail trace layout missing'
grep -q 'SV_FillLegacyTrace' runtime/server/gameplay/sv_game.c || fail 'server retail trace marshal missing'
