...
check "$LIT" 'matClass == CLASS_TRANSMISSION' 'deferred lighting detects transmission class'
check "$LIT" 'Transmission/refraction stay Forward+ owned' 'transmission class remains Forward+ owned'
...
check "$PIPE" 'deferred_unlit_base_strength = 0.0f' 'mode 1 does not bake handoff into specialization (runtime pbrDebugMode.y only)'
check "$DGB" 'vk_deferred_lighting_path_ready' 'opaque handoff gated on deferred path readiness'
check "$DGB" 'frame_capture_ok' 'per-frame albedo capture success flag'
check "$DGB" 'frame_lighting_ok' 'per-frame lighting dispatch success flag'
check "$DGB" 'skip composite REPLACE' 'composite fails open when capture/lighting incomplete'
