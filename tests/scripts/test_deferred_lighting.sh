...
check "$LIT" 'roughness = texture( materialTex, uv ).g' 'deferred lighting reads G-buffer roughness'
check "$LIT" 'metalness = texture( materialTex, uv ).r' 'deferred lighting reads G-buffer metalness'
