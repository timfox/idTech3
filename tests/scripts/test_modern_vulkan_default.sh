...
grep -q 'color     : post=%d tonemap=%d' "$TR_INIT_DIAG" || fail "renderer_status must expose color grading state"
grep -q 'PostFX_GetGradeHue' "$TR_INIT_DIAG" || fail "renderer_status must expose hue grading state"
