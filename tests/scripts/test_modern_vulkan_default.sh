...
grep -q 'seta r_renderMode 3' "$CFG" || fail "modern Vulkan must use Unified Clustered (mode 3)"
...
grep -q 'seta r_deferredLighting 1' "$CFG" || fail "modern Vulkan must enable deferred lighting for mode 3"
grep -q 'seta r_deferredUnlitBase 1' "$CFG" || fail "modern Vulkan must enable deferred unlit base"
grep -q 'seta r_aaMode 2' "$CFG" || fail "modern Vulkan must default r_aaMode 2 (SMAA 1x)"
grep -q 'seta r_taa 0' "$CFG" || fail "modern Vulkan must default Temporal Reconstruction off (SMAA baseline)"
grep -q 'seta r_ext_smaa 1' "$CFG" || fail "modern Vulkan must enable SMAA"
grep -q 'seta r_taaMotionVectors 1' "$CFG" || fail "modern Vulkan must enable motion-vector scaffolding"
grep -q 'seta r_deferredMaterialClassify 1' "$CFG" || fail "modern Vulkan must enable deferred material classify"
grep -q 'seta r_visibilityBuffer 1' "$CFG" || fail "modern Vulkan must enable visibility buffer for classify"
