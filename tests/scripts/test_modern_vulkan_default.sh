...
grep -q 'seta r_taa 1' "$CFG" || fail "modern Vulkan mode 3 must default TAA on (motion vectors)"
grep -q 'seta r_taaMotionVectors 1' "$CFG" || fail "modern Vulkan must enable TAA motion vectors"
