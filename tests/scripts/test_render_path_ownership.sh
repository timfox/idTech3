...
check "$ROOT/renderers/vulkan/shaders/glsl/deferred_lighting_common.glsl" 'attenPointLight\|attenSpotLight' 'deferred evaluates actual light bounds'
check "$ROOT/docs/RENDERER_PATH_OWNERSHIP.md" 'Directional sun lighting remains a separate owner' 'sun ownership is explicit'
...
check "$MODE_C" 'unified_clustered_raster' 'mode profile names certified raster'
