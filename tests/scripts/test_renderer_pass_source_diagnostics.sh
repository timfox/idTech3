...
check "$TR_DIAG" 'graph     : observed=%u compiled=%u deps=%u frameViol=%u totalViol=%u' 'renderer_status prints render graph health summary'
...
check "$TR_DIAG" 'rendergraph: observed=%u compiled=%u deps=%u frameViol=%u totalViol=%u' 'renderer_subsystems prints render graph counts'
