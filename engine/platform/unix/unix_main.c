...
static void tty_WriteFd( int fd, const void *buffer, size_t length )
{
...
{
	tty_WriteFd( STDOUT_FILENO, "\b \b", 3 );
}
...
static void tty_Show( void )
{
	...
	if ( ttycon_hide > 0 )
	{
		...
		if ( ttycon_hide == 0 )
		{
			tty_WriteFd( STDOUT_FILENO, "]", 1 ); // -EC-

			if ( tty_con.cursor > 0 )
			{
				tty_WriteFd( STDOUT_FILENO, tty_con.buffer, tty_con.cursor );
			}
		}
	}
}
...
void NORETURN Sys_Exit( int code )
{
	...
	_exit( code );
#else
	/* Use exit() for backtrace on error exits; do not assert on non-zero code */
	exit( code );
#endif
}
	...
	if ( ttycon_on )
	{
		...
		if (avail != -1)
		{
			...
			if (key && key < ' ')
			{
				...
				{
					tty_WriteFd( STDOUT_FILENO, "\x1b" "c]", 3 );
					if ( tty_con.cursor )
					{
						tty_WriteFd( STDOUT_FILENO, tty_con.buffer, tty_con.cursor );
					}
			...
			tty_WriteFd( STDOUT_FILENO, &key, 1 );
		}
		return NULL;
	}
...
tty_WriteFd( STDERR_FILENO, printmsg, len );
