...
- **SEGA (hi-res FLUX)**: Training-free attention rescaling for 4096²+ images: **`sega_generate`**, **`sega_status`** / **`sega_cancel`**, **`sega_view`**. Set **`cl_sega_enable` 1** and **`cl_sega_repo`** (vendored copy at **`external/sega`**; see **`docs/SEGA.md`**). Requires Linux + NVIDIA GPU; PyTorch runs out-of-process via **`sega_flux_generate.py`**.
