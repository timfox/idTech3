...
- **Spec-energy hi-res FLUX** (Spectral-Energy Guided Attention): Training-free attention rescaling for 4096²+ images: **`spec_energy_generate`**, **`spec_energy_status`** / **`spec_energy_cancel`**, **`spec_energy_view`**. Set **`cl_spec_energy_enable` 1** and **`cl_spec_energy_repo`** (vendored copy at **`external/flux_spec_energy`**; see **`docs/SPEC_ENERGY.md`**). Requires Linux + NVIDIA GPU; PyTorch runs out-of-process via **`spec_energy_flux_generate.py`**.
