...
- **World residency planner**: **`r_openWorldResidency` 1** — budgeted collision/nav/sprite cardinality with bounded per-frame swaps (replaces radius disk). MP: **`sv_openWorldResidency` 1**; client nav clamp: **`cl_openWorldResidencyNavLocal`**. See **`docs/WORLD_RESIDENCY.md`**. **`ctest -R test_openworld_residency`**.
- **Sector graph reachability**: **`r_graphStreamReach` 1** + **`r_graphStreamHops`** — k-hop BFS filter on residency candidates; optional Vulkan compute **`r_graphCompute` 1`** (client, **`vid_restart`**). Console: **`graph_reach_test`**, **`graph_bfs_status`**. See **`docs/GRAPH_COMPUTE.md`**. **`ctest -R unit_sector_graph`**.
