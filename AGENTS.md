...
- **Billboard / flipbook / imposter (engine-native)**: Map entities **`misc_*`** parsed on **`LoadWorld`** (`r_spriteProps 1`). Server registers shader paths in **`CS_ENGINE_SPRITE_SHADERS`** (`sv_engineSprites 1`). Networked ents: **`cl_engineSprites 1`**, **`modelindex`** = shader CS index, **`EF_*`** flags. Console: **`sprite_spawn`**. Demo: **`demo_sprites.cfg`**.
