...
void SCR_Init( void ) {
...

	{
		...
		Cvar_SetDescription( cl_builtInTtfConsole,
			"When 1, draw console/notify with FreeType (r_font / r_consoleFont). Default 0 uses the legacy 8x16 bitmap charset (Spine stable)." );
	}
	...
	{
		cvar_t *spacing = Cvar_Get( "r_fontConsoleLineSpacing", "0.2", CVAR_ARCHIVE );
		Cvar_CheckRange( spacing, "0.0", "0.25", CV_FLOAT );
		Cvar_SetDescription( spacing,
			"Additional TrueType console line leading as a fraction of the glyph cell height (0.2 = more readable spacing)." );
	}
