...
typedef struct {
	wrCellKey_t slots[WR_CELL_SET_SIZE];
	qboolean    used[WR_CELL_SET_SIZE];
	int         count;
} wrCellSet_t;
...
static uint32_t WR_CellHash( int cellX, int cellY ) {
	return (uint32_t)( ( cellX * 73856093 ) ^ ( cellY * 19349663 ) );
}

static void WR_CellSetClear( wrCellSet_t *set ) {
	Com_Memset( set->used, 0, sizeof( set->used ) );
	set->count = 0;
}

static void WR_CellSetInsert( wrCellSet_t *set, int cellX, int cellY ) {
	uint32_t hash;
	int i;
	int probe;

	if ( set->count >= WR_CELL_SET_SIZE ) {
		return;
	}

	hash = WR_CellHash( cellX, cellY );
	probe = (int)( hash % WR_CELL_SET_SIZE );
	for ( i = 0; i < WR_CELL_SET_SIZE; i++ ) {
		int idx = ( probe + i ) % WR_CELL_SET_SIZE;

		if ( !set->used[idx] ) {
			set->slots[idx].cellX = cellX;
			set->slots[idx].cellY = cellY;
			set->used[idx] = qtrue;
			set->count++;
			return;
		}
		if ( WR_CellEqual( set->slots[idx].cellX, set->slots[idx].cellY, cellX, cellY ) ) {
			return;
		}
	}
}

static qboolean WR_CellSetContains( const wrCellSet_t *set, int cellX, int cellY ) {
	uint32_t hash;
	int i;
	int probe;
...
static void WR_CellSetBuildFromCurrent( wrCellSet_t *set, const wrLayerState_t *st ) {
	int i;

	WR_CellSetClear( set );
	for ( i = 0; i < st->currentCount; i++ ) {
		WR_CellSetInsert( set, st->current[i].cellX, st->current[i].cellY );
	}
}

static void WR_CellSetBuildFromCells( wrCellSet_t *set, const worldResidencyCell_t *cells, int count ) {
	int i;

	WR_CellSetClear( set );
	for ( i = 0; i < count; i++ ) {
		WR_CellSetInsert( set, cells[i].cellX, cells[i].cellY );
	}
}

static qboolean WR_InCurrent( const wrLayerState_t *st, const wrCellSet_t *currentSet,
	int cellX, int cellY ) {
	if ( currentSet && currentSet->count > 0 ) {
		return WR_CellSetContains( currentSet, cellX, cellY );
	}
	{
		int i;
...
static qboolean WR_InSetHash( const wrCellSet_t *set, int cellX, int cellY ) {
	return WR_CellSetContains( set, cellX, cellY );
}

static qboolean WR_ServerAllows( const wrCellSet_t *allowSet, int cellX, int cellY ) {
	if ( s_serverAllowCount <= 0 ) {
		return qtrue;
	}
	if ( allowSet && allowSet->count > 0 ) {
		return WR_CellSetContains( allowSet, cellX, cellY );
	}
	{
		int i;
...
static void WR_AddCandidateUnique( worldResidencyCandidate_t *candidates, int *candidateCount,
	wrCellSet_t *candidateSet, int cellX, int cellY, float score, int regionId ) {
	int i;

	if ( candidateSet && WR_CellSetContains( candidateSet, cellX, cellY ) ) {
	...
	if ( candidateSet ) {
		WR_CellSetInsert( candidateSet, cellX, cellY );
	}
	( *candidateCount )++;
}
...
static void WR_EnumerateCandidatesAround( const vec3_t viewOrigin, float loadRadius,
	...
	wrLayerState_t *st, const wrCellSet_t *currentSet, const wrCellSet_t *allowSet,
	worldResidencyCandidate_t *candidates, int *candidateCount, wrCellSet_t *candidateSet ) {
	int centerX, centerY;
	...
	for ( y = minY; y <= maxY; y++ ) {
		for ( x = minX; x <= maxX; x++ ) {
			...
			sticky = WR_InCurrent( st, currentSet, x, y );
			...
			WR_AddCandidateUnique( candidates, candidateCount, candidateSet, x, y, score, regionId );
		}
	}
}
...
static void WR_BuildTarget( worldResidencyLayer_t layer, const vec3_t viewOrigin,
	...
	wrCellSet_t currentSet;
	wrCellSet_t allowSet;
	wrCellSet_t candidateSet;
	...
	WR_EnumerateCandidatesAround( viewOrigin, loadRadius, unloadRadius, sectorSize,
		layer, st, &currentSet, &allowSet, candidates, &candidateCount, &candidateSet );
...
static void WR_ComputePendingDelta( worldResidencyLayer_t layer ) {
	...
	WR_CellSetBuildFromCurrent( &currentSet, st );
	WR_CellSetClear( &targetSet );
	for ( i = 0; i < st->targetCount; i++ ) {
		WR_CellSetInsert( &targetSet, st->target[i].cellX, st->target[i].cellY );
	}

	for ( i = 0; i < st->targetCount; i++ ) {
		if ( !WR_InSetHash( &currentSet, st->target[i].cellX, st->target[i].cellY ) ) {
	...
	for ( j = 0; j < st->currentCount; j++ ) {
		if ( !WR_InSetHash( &targetSet, st->current[j].cellX, st->current[j].cellY ) &&
			st->pendingRemoveCount < WORLD_RESIDENCY_MAX_SET ) {
