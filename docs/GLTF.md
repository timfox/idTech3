...
glTF / GLB runs on the **Vulkan** renderer: `.gltf` and `.glb` are registered in `tr_model.c` and loaded via **cgltf** (`tr_model_gltf.c`). The full path includes optional **device VBOs**, **PBR GPU** skin/morph (`r_gltfGpu`), and qtangent recompute on CPU tess when needed.
...
- **Materials (partial)**: metallic-roughness base color, normal map, metallic-roughness texture, emissive, occlusion; factors and texture **URIs** are read. Base color drives `RE_RegisterShaderNoMip` on the primitive.
