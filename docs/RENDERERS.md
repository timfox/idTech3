...
- **Vulkan RTX**: demo path with `USE_VULKAN_RTX=ON`, `r_rtx` / `r_rtxDemo`, world BLAS; optional **`r_rtxEntities`** packs MD3 LOD0 + bind-pose IQM + static glTF (AABB for skinned/MDR/fail) into the entity BLAS. See [RENDERERS_FUTURE.md](RENDERERS_FUTURE.md).
