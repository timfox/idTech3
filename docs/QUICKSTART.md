...
- **Native game DLLs not found** — The engine looks under `baseq3`/`base` in `modules/` and `vm/` for names like `uix86_64.dll` and **`ui.x86_64.dll`** (same for `cgame` / `qagame`). If load still fails, run with `+set com_nativeLibraryDebug 1` to print the full path and the Windows loader error for each attempt.
