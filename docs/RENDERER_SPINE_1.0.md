...
- Late-post DEVICE_LOST / pass_diag (TAA→luminance→gamma) + combo soft-demote landed; GPU-walk black-frame class next
- Focus/minimize presentation restore hook + restore soft-fail landed; alt-tab/fullscreen matrix still manual
- OIT × TAA: one-shot warnings + soft-demote when weapon-after off; experimental stack allowed with weapon-after + `r_spineValidate 1`
