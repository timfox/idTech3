...
7. G-buffer / Ambient Visibility lifecycle: `./scripts/gbuffer_av_lifecycle_check.sh` (also via `spine_stability_check.sh`). Console: `deferred_gbuffer_status`, `ambient_visibility_status`, `havenrp_renderer_status`. Soft-fail create paths (fill/lighting/composite/debug); `r_dgbFailInject` recovers when cleared; `r_avFailInject` demotes AO to `legacy_ssao` without killing G-buffer fill (`history` thrash-resets AV temporal only). **Do not** enable `r_ambientVisibilityMode 4` in quality until the GPU matrix in that script’s footer passes.
