...
**`r_renderMode`** (latched, `vid_restart`): **0** classic forward (projector path; `r_forwardPlus` may still be on), **1** deferred (**lighting pass not implemented**; forward fallback). With **`r_deferredGBuffer 1`**, full-res albedo/normal/material RTs are allocated (`vk_attachments.c`); nothing writes them yet. **2** Forward+ primary (`tr_render_mode_vk.c` forces `r_forwardPlus` / `r_forwardPlusShade`).
