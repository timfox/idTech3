...
| **Vulkan RTX** | Shipping RT foundation | `USE_VULKAN_RTX=ON` + `r_rtx` / `r_rtxDemo`. World BLAS packs faces + trisoups + **SF_GRID**; **entity BLAS** packs MD3 LOD0 + **CPU-skinned IQM/MDR** + **static/CPU-skinned glTF** (`r_rtxEntities 1`) with AABB for pack failures (`rtx_status` mesh/proxy reasons). **Per-primitive albedo + geometric normal SSBO** for world and entity hits (Hybrid1/pathtrace/Surfel); world albedo uses **shader avgColor / UV-centroid thumbs** when **`r_rtxWorldMaterials`** / **`r_rtxWorldUvSample`** (defaults 1; fallback vertex color); entity albedo uses **shader/texture averages** when **`r_rtxEntityMaterials 1`**, or **UV-centroid thumb samples** when **`r_rtxEntityUvSample 1`** (pack-time; not hit-shader bindless yet). Entity BLAS **UPDATE** when prim count stable (`r_rtxEntityBlasUpdate`). **Surfel GI (GIBS)** via **`r_surfelGi`** + ray query; **Hybrid1↔Surfel fusion** (`r_surfelGi_hybrid1Fusion`, default on) hands irradiance to Hybrid1 composite. **Hybrid1 is the production RT lighting path** (`r_hybrid1` + **`r_hybrid1Quality`** 1–3 presets); plain `r_rtx` demo remains diagnostic. |
