...
| **Vulkan RTX** | Shipping RT foundation | `USE_VULKAN_RTX=ON` + `r_rtx` / `r_rtxDemo`. World BLAS packs faces + trisoups + **SF_GRID**; **entity BLAS** packs MD3 LOD0 + **CPU-skinned IQM/MDR** + **static/CPU-skinned glTF** (`r_rtxEntities 1`) with AABB for pack failures (`rtx_status` mesh/proxy reasons, `cpuskin=` / `skinfail=` / `proxy_rate=%`). **Per-primitive albedo + geometric normal SSBO** for world and entity hits (Hybrid1/pathtrace/Surfel/RcGI); world albedo uses **shader avgColor / UV-centroid thumbs** when **`r_rtxWorldMaterials`** / **`r_rtxWorldUvSample`** (defaults 1; fallback vertex color; optional **`r_rtxWorldAlbedoMode 1`** = material×vertex); entity albedo uses **shader/texture averages** when **`r_rtxEntityMaterials 1`**, or **UV-centroid thumb samples** when **`r_rtxEntityUvSample 1`** (pack-time; lightingStage-preferred via shared **`vk_rtx_material`**; not hit-shader bindless yet). Entity BLAS **UPDATE** when prim count stable (`r_rtxEntityBlasUpdate`). **Surfel GI (GIBS)** via **`r_surfelGi`** + ray query; **Radiance Cache GI (RcGI)** via **`r_rcgi`** (cascaded light grid + spatial-hash cache + probe atlas + SH gather — see **`docs/RADIANCE_CACHE_GI.md`**); Hybrid1↔RcGI/Surfel fusion hands irradiance to Hybrid1 composite (RcGI preferred when both on). **Hybrid1 is the production RT lighting path** (`r_hybrid1` + **`r_hybrid1Quality`** 1–3 presets); plain `r_rtx` demo remains diagnostic. |
...
2. **Acceleration structures**
   ...
   - **Per-primitive albedo + geometric normal SSBO**: RGB/normals from BSP faces (plane), triangle soups, and SF_GRID; world RGB prefers diffuse UV thumbs / avgColor (`r_rtxWorldMaterials` / `r_rtxWorldUvSample`) else vertex colors; bound for Hybrid1 / pathtrace / Surfel GI hits. **D2 Phase A scaffold:** Hybrid1 bindings 15/16 + `vk_rtx_bindless` prim-material SSBO (all `INVALID`); true hit UV sampling still off — see **`docs/RTX_HIT_SHADER_UV.md`**.
