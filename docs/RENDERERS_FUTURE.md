...
| **Vulkan RTX** | Chocolate RT foundation | `USE_VULKAN_RTX=ON` + `r_rtx` / `r_rtxDemo`. World BLAS packs faces + trisoups + **SF_GRID**; **entity BLAS** packs MD3 LOD0 mesh (`r_rtxEntities 1`) with AABB fallback for IQM/glTF/MDR (`rtx_status` proxy reasons). Per-primitive albedo SSBO for Hybrid1/pathtrace hits. **Hybrid1 is the production RT lighting path**; plain `r_rtx` demo remains diagnostic. |
...
2. **Acceleration structures**
   - **BLAS**: World BSP faces + trisoups + **SF_GRID** (LOD via `r_lodCurveError`); **entity BLAS** from MD3 LOD0 frame-lerped mesh (`r_rtxEntities`, `r_rtxEntityTriCap`) with AABB proxy fallback for non-MD3 (IQM/glTF/MDR); `rtx_status` reports proxy reason counts and TLAS UPDATE vs REBUILD reason
