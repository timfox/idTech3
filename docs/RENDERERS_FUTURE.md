...
| **Vulkan RTX** | Shipping RT foundation | `USE_VULKAN_RTX=ON` + `r_rtx` / `r_rtxDemo`. World BLAS packs faces + trisoups + **SF_GRID**; **entity BLAS** packs MD3 LOD0 + **CPU-skinned IQM** + **static glTF** (`r_rtxEntities 1`) with AABB for MDR, skinned glTF, and pack failures (`rtx_status` mesh/proxy reasons). **Per-primitive albedo + geometric normal SSBO** for Hybrid1/pathtrace/Surfel hits. **Surfel GI (GIBS)** via **`r_surfelGi`** + ray query (albedo/normal hits, recycle, spatial hash). **Hybrid1 is the production RT lighting path** (`r_hybrid1` + **`r_hybrid1Quality`** 1–3 presets); plain `r_rtx` demo remains diagnostic. |
...
2. **Acceleration structures**
   - **BLAS**: World BSP faces + trisoups + **SF_GRID** (LOD via `r_lodCurveError`); **entity BLAS** from MD3 LOD0 frame-lerped mesh, **CPU-skinned IQM** (`R_IQMSkinPositions`), and static glTF (`skeleton.numJoints==0`) via `r_rtxEntities` / `r_rtxEntityTriCap`; AABB proxy for MDR, skinned glTF, and pack failures; `rtx_status` reports mesh kind + proxy reason counts and TLAS UPDATE vs REBUILD reason
