...
| **SqueezeMe** | 🔶 Demo path | **`r_squeezeme` 1** (latched): UV Gaussian **full-body avatars** with **linear distilled** pose correctives + **GCS** (paper arXiv:2412.15171). CPU bake → **Mobile-GS** splat. **`r_squeezeme_avatars` 1–3**. See **`docs/SQUEEZEME.md`**. |
...
| **Vulkan RTX** | 🔶 Demo path | With `USE_VULKAN_RTX=ON` and `r_rtx`>0 (latched): device enables KHR AS + ray-tracing pipeline + BDA. `r_rtxDemo` 1 (default): world BSP BLAS (all brush submodels `*0..*N-1`: SF_FACE + SF_TRIANGLES, capped by latched `r_rtxWorldPrimCap`) when a map is loaded, else a fallback triangle; raygen uses main-pass depth + `invViewProj` (Vulkan Y-flip projection, first-person projection when active); trace resolution matches **`vk_get_render_target_*`** (FBO / `r_renderScale`). **Closest-hit tints by `r_rtx` (1–3)** for hybrid visualization; entity proxies (`r_rtxEntities` 1) tint via **`gl_InstanceCustomIndexEXT`**. World BLAS cached per map; entity TLAS uses **`r_rtxTlasUpdate` 1** (UPDATE when instance count stable). Console **`rtx_status`**. Trace after main/post-bloom (Hybrid1/Raygun take priority), blit into `color_image`. **`r_rtxEntities` 1** (default **0**): entity proxy BLAS + TLAS (`r_rtxEntityCap`). Real lighting / full mesh BLAS still TODO. **`scripts/compile_shaders.sh --apply` regenerates `vk_rtx_demo_spirv.inc` from `glsl/rtx_demo.*`.** |
...
| **Hybrid Rendering 1** | 🔶 Experimental | **`r_hybrid1` 1** (latched): Granja/Pereira (2021) **hybrid raster + RT** — separate 1-SPP shadow/specular/**diffuse**, **SVGF** temporal + variance color clamp, adaptive separable A-trous, Reinhard specular, **IBL** cubemap on secondary hits, composite + optional **TAA** (`vk_hybrid1.c`, `glsl/hybrid1/*`). Requires **`r_rtxDemo` 1**, **`r_fbo` 1**, deferred G-buffer recommended. Replaces `r_rtx` demo pass when active. See **`docs/HYBRID_RENDERING1.md`**. |
...
| **Raygun RT** | 🔶 Demo path | **`r_raygun` 1** (latched): Hirsch/Thoman (arXiv:2001.09792) **primary + shadow/reflection/refraction** traces over world TLAS, optional **FXAA** compute. Requires **`r_rtxDemo` 1**, **`r_fbo` 1**. Replaces `r_rtx` demo when active (Hybrid1 priority). See **`docs/RAYGUN.md`**. |
