...
| **Vulkan RTX** | Shipping RT foundation | `USE_VULKAN_RTX=ON` + `r_rtx` / `r_rtxDemo`. World BLAS packs faces + trisoups + **SF_GRID**; **entity BLAS** packs MD3 LOD0 + **bind-pose IQM** + **static glTF** (`r_rtxEntities 1`) with AABB for skinned IQM/glTF, MDR, and pack failures (`rtx_status` mesh/proxy reasons). **Per-primitive albedo + geometric normal SSBO** for Hybrid1/pathtrace/Surfel hits. **Surfel GI (GIBS)** via **`r_surfelGi`** + ray query (albedo/normal hits, recycle, spatial hash). **Hybrid1 is the production RT lighting path**; plain `r_rtx` demo remains diagnostic. |
...
2. **Acceleration structures**
   - **BLAS**: World BSP faces + trisoups + **SF_GRID** (LOD via `r_lodCurveError`); **entity BLAS** from MD3 LOD0 frame-lerped mesh, bind-pose IQM (`num_joints==0`), and static glTF (`skeleton.numJoints==0`) via `r_rtxEntities` / `r_rtxEntityTriCap`; AABB proxy for skinned IQM/glTF, MDR, and pack failures; `rtx_status` reports mesh kind + proxy reason counts and TLAS UPDATE vs REBUILD reason
   - **Per-primitive albedo + geometric normal SSBO**: RGB/normals from BSP faces (plane), triangle soups, and SF_GRID; bound for Hybrid1 / pathtrace / Surfel GI hits
   - **Production lighting**: prefer **Hybrid1** (`r_hybrid1`) over the plain `r_rtx` tint demo
