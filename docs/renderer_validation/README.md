...
| Goal | What to do |
|------|------------|
| **Modern profile source contract** | `./tests/scripts/test_modern_renderer_profile_runtime.sh source` or `ctest -R test_modern_renderer_profile_runtime --output-on-failure`. This is headless-safe and verifies the Forward+ + G-buffer sidecar + PBR + HDR + SMAA (`r_aaMode 2`) default plus diagnostics wiring, including advanced lighting / neural GI compatibility warnings. |
| **Modern profile client smoke** | `IDTECH3_RENDERER_RUNTIME_REQUIRED=1 ./tests/scripts/test_modern_renderer_profile_runtime.sh runtime` on a machine with a built Vulkan client, `DISPLAY`, and working Vulkan. This executes `modern_vulkan.cfg` and checks `renderer_profile`, `renderer_status`, `renderer_subsystems`, and `renderer_compatibility`, including the `lighting`, `temporal`, and `gi/neural` status rows. Use `all` instead of `runtime` to include the source/packaging contract in the same run. |
| **Tier B locally** | `GAME_BASE=/abs/path/to/base ./scripts/renderer_regression_check.sh` then same with `renderer_regression_maps.sh`. If `GAME_BASE` is unset, `./scripts/resolve_renderer_tiers.sh --tier B` uses the GPL-friendly `docs/renderer_validation/devdata/rtest_base` pack for dedicated map-load automation. |
...
| [TEMPLATE_TIER_C.md](TEMPLATE_TIER_C.md) | Copy per session; record date, commit, maps, Vulkan GPU pass, validation-layer result |
