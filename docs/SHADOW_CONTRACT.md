...
WBOIT accum (`oit_accum.frag`) samples **CSM via shadow-contract set 3** (`ShadowContract_SampleCSM_FromRecords` — cascade far/near/blend packed in `filterParams.yzw`; BestFit fallback) and modulates the directional sun term; consumer noted as `oit`.
