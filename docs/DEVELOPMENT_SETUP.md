...
```bash
# USE_STEAM defaults ON; SDK auto-detected at /home/tim/SteamWorks/sdk when present
./scripts/compile_engine.sh vulkan
# Opt out: ./scripts/compile_engine.sh vulkan no-steam
# Equivalent:
...
- **USE_STEAM** (default **ON**): Achievements, overlay, rich presence, Steam Deck auto-detection, Steam Input status. When Deck is detected, `base/steamdeck.cfg` is auto-exec'd. Without an SDK the stub path still compiles.
- **USE_OPENHMD** (default **ON**): OpenHMD VR head tracking + software stereo. Loads `libopenhmd.so` at runtime (`dlopen`); install `libopenhmd0` to use. See `docs/OPENHMD.md`.
