...
| r_renderMode 1 | `tr_init.c`, `tr_render_mode_vk.c` | **`r_deferredLighting 1`**: G-buffer fill + Forward+ tile lights with Disney/Burley **Fd** + GGX specular; **`r_deferredUnlitBase 1`** additive dynamic on static base; latches `r_forwardPlusShade` 0. |
...
| 2026-04-11 | Vulkan / future GPU | **Virtual geometry meshlets** now default in the stable renderer profile (`r_virtualGeometry 1`, meshlet MDI + screen LOD). **`r_vk_meshShaderNV`** remains default 0 and only enables optional NVIDIA mesh-shader capability for experiments/diagnostics while portable indexed MDI stays production. **DLSS:** not in-repo; startup log documents use `r_renderScale` / driver scaling. |
...
6. **RTX long-term**: True hit-shader UV/bindless texturing — Phase A scaffold in **`docs/RTX_HIT_SHADER_UV.md`** (pack-time thumbs + SSBO fallback until Phase A.1 sampling).
