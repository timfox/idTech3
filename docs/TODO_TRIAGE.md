...
| Vulkan RTX entities | `vk_rtx_entities.c` | **PARTIAL**: MD3 LOD0 + **CPU-skinned IQM/MDR** + static/CPU-skinned glTF mesh BLAS; AABB for pack failures. Real lighting attrs via entity albedo/normal SSBOs. World BSP BLAS + demo blit when `r_rtxDemo` 1. **`USE_VULKAN_RTX` OFF**: `#else` stubs. See `docs/RENDERERS_FUTURE.md`. |
| Surfel GI (GIBS) | `vk_surfel_gi.c` | **PARTIAL**: Chocolate RTX — spawn/update/hash/resolve/composite; world + **entity** albedo/normal SSBOs + stale recycle + fixed-bucket spatial hash + **Hybrid1 fusion** (`r_surfelGi_hybrid1Fusion`). Still open: denser budgeting. See `docs/SURFEL_GI.md`. |
...
6. **RTX long-term**: real material sampling on entity hits; Hybrid1↔Surfel fusion (MD3/IQM/glTF/MDR mesh AS + entity attrs landed).
