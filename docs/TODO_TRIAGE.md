...
| Vulkan RTX entities | `vk_rtx_entities.c` | **PARTIAL**: MD3 LOD0 + **CPU-skinned IQM** + static glTF mesh BLAS; AABB for MDR / skinned glTF / pack failures. Real lighting still open. World BSP BLAS + demo blit when `r_rtxDemo` 1. **`USE_VULKAN_RTX` OFF**: `#else` stubs. See `docs/RENDERERS_FUTURE.md`. |
