...
| Vulkan RTX entities | `vk_rtx_entities.c` | **PARTIAL**: MD3 LOD0 + **CPU-skinned IQM** + static/CPU-skinned glTF mesh BLAS; AABB for MDR / pack failures. Real lighting still open. World BSP BLAS + demo blit when `r_rtxDemo` 1. **`USE_VULKAN_RTX` OFF**: `#else` stubs. See `docs/RENDERERS_FUTURE.md`. |
| Surfel GI (GIBS) | `vk_surfel_gi.c` | **PARTIAL**: Chocolate RTX — spawn/update/hash/resolve/composite; world albedo+normal SSBOs + stale recycle + fixed-bucket spatial hash. Still open: Hybrid1 fusion, entity hit attrs, denser budgeting. See `docs/SURFEL_GI.md`. |
...
6. **RTX long-term**: real lighting + entity albedo/normal attrs; MDR mesh BLAS still open (MD3/IQM/glTF mesh AS landed).
