...
- The Vulkan renderer is still **forward-only** (no deferred G-buffer path). Optional tile culling uses **`r_forwardPlus`** (default **1**), not a separate render-mode switch.
...
- **Fix doc/code drift and make the forward-only architecture explicit** - ✅ RENDERERS.md states **forward** main pass and **Vulkan Forward+** via **`r_forwardPlus`** (default **1**; set **0** to disable). Removed unused **`r_renderMode`** placeholder cvar.
