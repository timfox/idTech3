...
**Streaky/smudged visuals when dead**: Volumetric fog and TAA stationary feedback accumulate when the death camera is nearly static (HUD/crosshair stay sharp). `r_volumetricFogSkipStatic` (default 1) skips volumetrics **and** disables TAA for the same ~0.5 s near-static window so history cannot smear sky/fog into streaks. Set to 0 to always keep fog/TAA history. If artifacts persist, try `r_volumetricFog 0` / `r_taa 0` to confirm, or `r_volumetricFogForceCameraCut 1` to force an immediate reset (debug).
...
**Deferred × TAA anti-ghost** (mode 1/3 with `r_deferredLighting 1`): deferred GGX specular soft-caps relative to diffuse before writing lit HDR; host TAA history weight is reduced when deferred lighting (and further when `r_deferredSpecular 1`) is active; TAA reactive prefers current on bright highlight trails and tightens YCoCg clamp on high-luma pixels. Stable mode 2 (`r_deferredLighting 0`, `r_taa 0`) is unaffected. Deferred overlay keeps TAA off by default and pins `r_temporalWeaponAfterTaa` / variance-clip / disocclusion for opt-in stacks.

**Echoing glow / silhouette trails**: Heuristic reactive (near-weapon depth, luma flash, **history-bleed** when former bright pixels overpaint dark geo) always runs whenever TAA is on — it is no longer gated behind `r_temporalReactiveMask` (that cvar only enables the stamped OIT/transparent mask). Temporal recon overlay uses `r_aaMode 4` (no post-TAA SMAA cleanup) with a lower history weight to avoid white skyline / HOST-banner ghosts.
