...
// Expose syscall so other cgame units (e.g., cg_main) can verify initialization.
...
float	trap_R_Font_Height(fontInfo_t *font, float scale) {
	int temp = syscall( CG_R_FONT_HEIGHT, font, PASSFLOAT(scale) );
	return *(float*)&temp;
}

float	trap_R_Font_Width(const char *text, float scale, fontInfo_t *font) {
	int temp = syscall( CG_R_FONT_WIDTH, text, PASSFLOAT(scale), font );
	return *(float*)&temp;
}

void	trap_R_Font_DrawString(float x, float y, const char *text, const vec4_t color, float scale, fontInfo_t *font, int style) {
	syscall( CG_R_FONT_DRAWSTRING, PASSFLOAT(x), PASSFLOAT(y), text, color, PASSFLOAT(scale), font, style );
}
...
void	trap_R_AddParticle( const vec3_t origin, const vec3_t velocity, const vec3_t color, float size, float life, qhandle_t shader ) {
	syscall( CG_R_ADDPARTICLE, origin, velocity, color, PASSFLOAT(size), PASSFLOAT(life), shader );
}
